...
async function getContentsShouldPurged() {
    const paths = ['/index.html', '/post/browser-on-browser/index.html', '/profile/index.html'];
                ...
                .map((asset) => {
                    ...
                    if (asset.tagName === 'LINK') {
                        ...
                        if (
                            !(
                                (
                                    href.startsWith('http') || href.startsWith('/cdn-cgi')
                                ) /* TODO ? */
                            ) /* TODO ? */
                        ) {
                            /* TODO */ return href;
                        ...
                        if (
                            !(
                                (src.startsWith('http') || src.startsWith('/cdn-cgi')) /* TODO ? */
                            ) /* TODO ? */
                        ) {
                            /* TODO */ return src;
                        }
    ...
    const allPurgeAssets = await Promise.all(
        allAssets.map(async (asset) => ((await shouldPurge(asset)) ? asset : undefined))
    );
