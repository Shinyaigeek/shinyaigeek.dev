...
declare function on(path: `/${p}`, handler: handler, children: undefined): Router;
declare function on(path: `/${p}`, handler: undefined, children: Router[]): Router;
...
export class Router {
    ...
    registerLanguage: (language: string, message: any) => void = (language, message) => {
        this.languages.add(language);
        i18n.load(language, message);
    };

    // TODO: lingui message is js
    registerDefaultLanguage: (language: string, message: any) => void = (language, message) => {
